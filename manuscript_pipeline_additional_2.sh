...
### to run additional analyses using                                  ##
### cross-mappability data computed with different settings           ##
### (configured by changing appropriate setting variables)            ##
...
#crossmap_fn="/work-zfs/abattle4/ashis/progres/crossmap/hg19_gencode19_75mer_75mer/cross_mappability.txt"
#mappability_fn="/work-zfs/abattle4/ashis/progres/crossmap/hg19_gencode19_75mer_75mer/gene_mappability/gene_mappability.txt"
crossmap_fn="/work-zfs/abattle4/ashis/progres/crossmap/hg19_gencode19_75mer_36mer_3mismatch/cross_mappability.txt"
mappability_fn="/work-zfs/abattle4/ashis/progres/crossmap/hg19_gencode19_75mer_36mer_3mismatch/gene_mappability/gene_mappability.txt"
...
#root_out_dir="$manuscript_analysis_dir/analysis_75mer_75mer_2mismatch"
root_out_dir="$manuscript_analysis_dir/analysis_75mer_36mer_3mismatch"
...
filtered_crossmap_dir="$root_out_dir/filtered_crossmap"
...
trnas_eqtl_dirs=("$inter_chr_trans_eqtl_dir")
trans_eqtl_fn_pfx=""
...
# original_meqtl_file_format="/work-zfs/abattle4/ashis/progres/misc/cross_mappability/manuscript_analysis/analysis/trans_eqtl/matrix_eqtls/#TISSUE#/#FILE#"
# original_meqtlmeta_file_format="/work-zfs/abattle4/ashis/progres/misc/cross_mappability/manuscript_analysis/analysis/trans_eqtl/matrix_eqtls/meqtl_meta_#TISSUE#.txt"
...
# do
#   echo "formatting gtex processed data - $tissue"
#   gtex_expr_fn="$gtex_expr_dir/${tissue}.v7.normalized_expression.bed.gz"
#   processed_expr_fn="$processed_expr_dir/${tissue}.v7.normalized_expression.bed"
#   gzip -cd $gtex_expr_fn | cut -f4- | awk '{FS="\t"} {OFS="\t"}{if (NR==1) {first=$1 ; $1="description"; print first "\t" $0} else {print $1 "\t" $0}} '  > $processed_expr_fn
# done
...
# do
#   echo $tissue
#   expr_fn="$processed_expr_dir/$tissue.v7.normalized_expression.bed"
#   cov_fn="$cov_dir/$tissue.v7.covariates.txt"
#   corrected_expr_fn="$corrected_expr_dir/$tissue.v7.corrected.bed"
...
Rscript explore_cross_mappability.R -cross "$crossmap_fn" -o "$crossmap_exploration_dir/$dist_plt_fn"
# TIME: 2 min
...
do
  ...
  Rscript random_cor_bet_cross_mappable_pairs.R -cross $crossmap_fn \
                                                ...
                                                -q "" \
                                                -sym FALSE \
                                                -sep '1,2,5,10,20,50,100,200,400,1e8' \
...
#   do
#     # split by chr
#     echo "chr$chr: splitting by chr"
#     chr_vcf_fn="${genotype_processing_dir}/chr${chr}.vcf"
#     tabix -h $genotype_vcf_fn $chr > $chr_vcf_fn
#     
#     # filter by maf
#     echo "chr$chr: filtering maf<=0.05"
#     maf_vcf_pfx="${genotype_processing_dir}/chr${chr}_maf_0.05"
#     maf_vcf_fn="$maf_vcf_pfx.vcf"
#     vcftools --vcf $chr_vcf_fn  --maf 0.05 --recode --out $maf_vcf_pfx 
#     mv "$maf_vcf_pfx.recode.vcf" $maf_vcf_fn
#     
#     # repeat masking
#     echo "chr$chr: repeat masking"
#     chr_repeat_fn="${genotype_processing_dir}/chr${chr}_repeat_mask.txt"
#     repeat_vcf_fn="${genotype_processing_dir}/chr${chr}_maf_0.05_repeat_masked.vcf"
#     awk -F $'\t' -v ch="chr$chr" '$6==ch {print substr($6, 4) "\t" $7 "\t" $8}' $repeat_mask_fn > $chr_repeat_fn
#     bcftools view -T ^"$chr_repeat_fn" -O v -o $repeat_vcf_fn $maf_vcf_fn
#     
#     # convert to 0-1-2 matrix
#     echo "chr$chr: making 012 matrix"
#     chr_genotype_matrix_pfx="${genotype_processing_dir}/chr${chr}_maf_0.05_repeat_masked"
#     vcftools --vcf $repeat_vcf_fn --012 --out $chr_genotype_matrix_pfx
#     
#     # create genotype file with row and column name
#     Rscript $crossmap_analysis_prog_dir/genotype_012_to_matrix.R \
#       -geno012 "$chr_genotype_matrix_pfx.012" \
#       -vcf $repeat_vcf_fn \
#       -o "$chr_genotype_matrix_pfx.012.txt"
#   
#   done
# fi
...
# meqtl_file_format="$original_meqtl_file_format"
# meqtlmeta_file_format="$meqtlmeta_file_format"
