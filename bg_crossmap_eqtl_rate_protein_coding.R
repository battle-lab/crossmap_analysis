### this script computes the background rate of cross-mappable eqtls, using only protein coding genes
...
args <- add_argument(args, '-o',
                     help='output file',
                     default='results/background_eqtl_crossmap_protein_coding.txt')
...
# PROTEIN_CODING: keep protein coding genes in gencode
...
# PROTEIN_CODING: filter crossmap to include protein-coding genes only
protein_coding_genes = gencode_df$gene_id
is_g1_coding = crossmap_df[,1] %in% protein_coding_genes
is_g2_coding = crossmap_df[,2] %in% protein_coding_genes
crossmap_df = crossmap_df[is_g1_coding & is_g2_coding, ]
...
#names(snps_near_gene) = rownames(gencode_df)
t2 = Sys.time()
t2 - t1
...
bg_crossmap_rate = sapply(expr_files, function(expr_fn){
  ...
  # PROTEIN_CODING: use protein coding genes only
  genes = intersect(genes, gencode_df$gene_id)
